...
void CFuncConveyor::UpdateSpeed( float speed )
{
	...
	int speedCode = static_cast<int>( fabs( speed ) * 16.0f );

	if( speed < 0.0f )
		pev->rendercolor.x = 1.0f;
	else
		pev->rendercolor.x = 0.0f;
...
void CFuncRotating::HurtTouch( CBaseEntity *pOther )
{
	...
	pev->dmg = pev->avelocity.Length() / 10.0f;
...
void CFuncRotating::RampPitchVol( int fUp )
{
	...
	vecCur = fabs( vecAVel.x != 0.0f ? vecAVel.x : ( vecAVel.y != 0.0f ? vecAVel.y : vecAVel.z ) );

	// get target angular velocity
	vecFinal = ( pev->movedir.x != 0.0f ? pev->movedir.x : ( pev->movedir.y != 0.0f ? pev->movedir.y : pev->movedir.z ) );
	vecFinal *= pev->speed;
...
void CFuncRotating::SpinDown( void )
{
		...
		( ( vecdir < 0.0f ) && ( vecAVel.x >= 0.0f && vecAVel.y >= 0.0f && vecAVel.z >= 0.0f ) ) )
	{
		...
		EMIT_SOUND_DYN( ENT( pev ), CHAN_STATIC, STRING( pev->noiseRunning /* Stop */ ),
				0.0f, 0.0f, SND_STOP, static_cast<int>( m_pitch ) );
