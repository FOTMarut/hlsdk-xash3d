...
void CEgon::Attack( void )
{
	...
	switch( m_fireState )
	{
		case FIRE_OFF:
		{
			...
			PLAYBACK_EVENT_FULL( flags, m_pPlayer->edict(), m_usEgonFire, 0.0, g_vecZero.asArray(), g_vecZero.asArray(), 0.0, 0.0, m_fireState, m_fireMode, 1, 0 );
		...
		case FIRE_CHARGE:
		{
			...
			{
				PLAYBACK_EVENT_FULL( flags, m_pPlayer->edict(), m_usEgonFire, 0, g_vecZero.asArray(), g_vecZero.asArray(), 0.0, 0.0, m_fireState, m_fireMode, 0, 0 );
				pev->fuser1 = 1000;
			}
...
void CEgon::EndAttack( void )
{
	...
	PLAYBACK_EVENT_FULL( FEV_GLOBAL | FEV_RELIABLE, m_pPlayer->edict(), m_usEgonStop, 0, m_pPlayer->pev->origin.asArray(), m_pPlayer->pev->angles.asArray(), 0.0, 0.0, bMakeNoise, 0, 0, 0 );
