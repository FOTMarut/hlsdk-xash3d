...
void CApache::Flight( void )
{
	...
	Vector vecEst = pev->origin + pev->velocity * 2.0f + gpGlobals->v_up * m_flForce * 20.0f - Vector( 0, 0, 384 * 2 );
	...
	else
	{
		...
		if( pPlayer )
		{
			...
			pitch = static_cast<int>( 100.0f + pitch / 50.0f );
...
void CApache::TraceAttack( entvars_t *pevAttacker, float flDamage, Vector vecDir, TraceResult *ptr, int bitsDamageType )
{
	...
	if( flDamage > 50.0f || ptr->iHitgroup == 1 || ptr->iHitgroup == 2 )
	{
		// ALERT( at_console, "%.0f\n", flDamage );
