...
void CGib::SpawnStickyGibs( entvars_t *pevVictim, Vector vecOrigin, int cGibs )
{
	...
	for( i = 0; i < cGibs; i++ )
	{
		...
		pGib->pev->body = RANDOM_LONG( 0.0f, 2.0f );
		...
		if( pevVictim )
		{
			...
			pGib->pev->origin.z = pevVictim->absmin.z + pevVictim->size.z * ( RANDOM_FLOAT( 0.0f, 1.0f ) );
			*/

			// make the gib fly away from the attack vector
			pGib->pev->velocity = -g_vecAttackDir;
			...
			if( pevVictim->health > -50.0f )
			{
				pGib->pev->velocity = pGib->pev->velocity * 0.7f;
			}
			else if( pevVictim->health > -200.0f )
			{
...
void CBaseMonster::GibMonster( void )
{
	...
	EMIT_SOUND( ENT( pev ), CHAN_WEAPON, "common/bodysplat.wav", 1.0f, ATTN_NORM );
...
void CBaseMonster::Killed( entvars_t *pevAttacker, int iGib )
{
	...
	EMIT_SOUND( ENT( pev ), CHAN_WEAPON, "common/null.wav", 1.0f, ATTN_NORM );
...
void CGib::WaitTillLand( void )
{
	...
	if( pev->velocity == g_vecZero )
	{
		...
		{
			// ok, start stinkin!
			CSoundEnt::InsertSound( bits_SOUND_MEAT, pev->origin, 384, 25.0f );
		}
	}
	else
	{
...
void CGib::BounceGibTouch( CBaseEntity *pOther )
{
	...
	else
	{
		...
		if( m_material != matNone && RANDOM_LONG( 0, 2 ) == 0 )
		{
			...
			CBreakable::MaterialSoundRandom( edict(), m_material, volume );
		}
	}
}
...
void CGib::StickyGibTouch( CBaseEntity *pOther )
{
	...
	pev->velocity = -tr.vecPlaneNormal;
...
{ 
	float force = damage * ( ( 32 * 32 * 72.0f ) / ( pev->size.x * pev->size.y * pev->size.z ) ) * 5.0f;
	...
	for( ULONG iShot = 1; iShot <= cShots; iShot++ )
	{
		...
		if( iTracerFreq != 0 && ( tracerCount++ % iTracerFreq ) == 0 )
		{
			...
			if( IsPlayer() )
			{
				...
				vecTracerSrc = vecSrc + Vector( 0, 0, -4 ) + gpGlobals->v_right * 2.0f + gpGlobals->v_forward * 16.0f;
			}
			else
			{
				vecTracerSrc = vecSrc;
		...
		if( tr.flFraction != 1.0f )
		{
			...
			case BULLET_NONE: // FIX
				pEntity->TraceAttack( pevAttacker, 50.0f, vecDir, &tr, DMG_CLUB );
				TEXTURETYPE_PlaySound( &tr, vecSrc, vecEnd, iBulletType );
				// only decal glass
		...
		// make bullet trails
		UTIL_BubbleTrail( vecSrc, tr.vecEndPos, static_cast<int>( ( flDistance * tr.flFraction ) / 64.0f ) );
	}
...
*/
	...
	for( i = 0; i < cCount; i++ )
	{
		vecTraceDir = -vecDir;// trace in the opposite direction the shot came from (the direction the shot is going)
