...
int CHudMOTD::Draw( float fTime )
{
	...
	char *ch = m_szMOTD;
	int xpos = ( ScreenWidth - gHUD.m_scrinfo.charWidths[static_cast<unsigned char>( 'M' )] * m_iMaxLength ) / 2;
	if( xpos < 30 )
		xpos = 30;
	int xmax = xpos + gHUD.m_scrinfo.charWidths[static_cast<unsigned char>( 'M' )] * m_iMaxLength;
	...
	while( *ch )
	{
		...
		for( next_line = ch; *next_line != '\n' && *next_line != 0; next_line++ )
			line_length += gHUD.m_scrinfo.charWidths[static_cast<unsigned char>( *next_line )];
		char *top = next_line;
		if( *top == '\n' )
			*top = 0;
		else
